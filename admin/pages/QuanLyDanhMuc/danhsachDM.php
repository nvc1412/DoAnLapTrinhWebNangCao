...
<div class="row">
    <div class="col-md-12">
        ...
        <table class='table table-bordered table-striped'>
            ...
            <tbody>
                <tr>
                    <form action="quantri.php?page_layout=themDM" method="post">
        ...
        <?php
            ...
            $sql = "SELECT * FROM danhmucsanpham ORDER BY id DESC LIMIT $perRow, $rowsPerPage";
            if($result = mysqli_query($conn, $sql)){

                //Phân trang
                $tongdanhmuc = mysqli_num_rows(mysqli_query($conn, "SELECT * FROM danhmucsanpham"));
                $tongsotrang = ceil($tongdanhmuc/$rowsPerPage);
                ...
                if(mysqli_num_rows($result) > 0){
                    echo "<table class='table table-bordered table-striped'>";
                        ...
                        while($row = mysqli_fetch_array($result)){
                            echo "<tr>";
                                echo "<form action='quantri.php?page_layout=suaDM' method='post'>";
                                ...
                                echo "<td>";
                                    ...
                                    echo "<a style='display: contents;' data-toggle='tooltip' href='quantri.php?page_layout=xoaDM&id=". $row['id'] ."'> <i title='Xóa danh mục' class='btn-xoa material-icons'>delete</i> </a>";
                                echo "</td>";
                                echo "</form>";
                            echo "</tr>";
                        }
...
<ul class="pagination">
    ...
    if($page<=$tongsotrang  && $page != 1){
        echo"<li><a href='quantri.php?page_layout=danhsachDM&page=".($page-1)."'>&laquo;</a></li>";
    }
                              
    for($i = $offset; $i <= min($offset + $so_trang_hien_thi - 1, $tongsotrang); $i++){ ?>
    <li><a <?= ($page==$i) ? "class='active'" : "" ?>
            href='quantri.php?page_layout=danhsachDM&page=<?= $i?>'><?= $i?></a></li>
    <?php }

    if($page>=1 && $page != $tongsotrang){
        echo"<li><a href='quantri.php?page_layout=danhsachDM&page=".($page+1)."'>&raquo;</a></li>";
    }
