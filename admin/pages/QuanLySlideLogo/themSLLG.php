...
if(isset($_POST["submit"])){
    ...
    $_SESSION['them_type'] = $_POST["type"];
    $_SESSION['them_position'] = $_POST["position"];
    $_SESSION['them_status'] = $_POST["status"];
    ...
    if(empty($_SESSION['error'])){
        ...
        $sql = "INSERT INTO slidelogo (name, type, url_image, position, status, description) VALUES ('$name', '$type', '$image_url', $position, '$status', '$description')";

        if (mysqli_query($conn, $sql)) {
            // Update thành công. Chuyển hướng đến trang đích
            unset($_SESSION['error']);
            unset($_SESSION['them_name']);
            unset($_SESSION['them_type']);
            unset($_SESSION['them_position']);
            unset($_SESSION['them_status']);
