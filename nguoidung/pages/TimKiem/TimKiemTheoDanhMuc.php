<?php
    ...
    $sql = "SELECT * FROM sanpham WHERE category_id = $search ORDER BY id DESC LIMIT $perRow, $rowsPerPage";

    if($result = mysqli_query($conn, $sql)){

        //Phân trang
        $tongsanpham = mysqli_num_rows(mysqli_query($conn, "SELECT * FROM sanpham WHERE category_id = $search"));
            ...
            <div class="col-sm-9 padding-right">
                ...
                <ul class="pagination">
                    ...
                    if($page<=$tongsotrang  && $page != 1){
                        echo"<li><a href='index.php?page_layout=TimKiemTheoDanhMuc&search=$search&name=$name&page=".($page-1)."'>&laquo;</a></li>";
                    }
                              
                    for($i=1; $i<=$tongsotrang; $i++){ ?>
                    <li <?= ($page==$i) ? "class='active'" : "" ?>><a
                            href='index.php?page_layout=TimKiemTheoDanhMuc&search=<?= $search ?>&name=<?= $name ?>&page=<?= $i?>'><?= $i?></a>
                    </li>
                    <?php }

                    if($page>=1 && $page != $tongsotrang){
                        echo"<li><a href='index.php?page_layout=TimKiemTheoDanhMuc&search=$search&name=$name&page=".($page+1)."'>&raquo;</a></li>";
                    }
