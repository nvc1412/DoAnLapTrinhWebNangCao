<?php
    ...
    if(isset($_POST['price']) && $_POST['price'] != ""){
        $min = explode(",",$_POST['price'])[0];
        $max = explode(",",$_POST['price'])[1];
    }else if(isset($_GET['min']) && isset($_GET['max'])){
        $min = $_GET['min'];
        $max = $_GET['max'];
    }else{
        // echo "<script> location.href = 'index.php?page_layout=CuaHang'; </script>";
    ...
    $sql = "SELECT * FROM sanpham WHERE price BETWEEN $min AND $max ORDER BY id DESC LIMIT $perRow, $rowsPerPage";

    if($result = mysqli_query($conn, $sql)){

        //Phân trang
        $tongsanpham = mysqli_num_rows(mysqli_query($conn, "SELECT * FROM sanpham WHERE price BETWEEN $min AND $max"));
        ...
        <div class="row">
            ...
            <div class="col-sm-9 padding-right">
                <div class="breadcrumbs">
                    <ol class="breadcrumb">
                        <li><a style="position: relative;" href="#">Danh mục</a></li>
                        <li class="active">Lọc giá: <?= number_format($min)." -> ". number_format($max)?></li>
                    </ol>
                </div>
                <div class="features_items">
                    <!--features_items-->
                    <h2 class="title text-center"><?=number_format($min)." -> ". number_format($max)?></h2>
                ...
                <ul class="pagination">
                    ...
                    if($page<=$tongsotrang  && $page != 1){
                        echo"<li><a href='index.php?page_layout=LocGia&min=$min&max=$max&page=".($page-1)."'>&laquo;</a></li>";
                    }
                              
                    for($i=1; $i<=$tongsotrang; $i++){ ?>
                    <li <?= ($page==$i) ? "class='active'" : "" ?>><a
                            href='index.php?page_layout=LocGia&min=<?= $min ?>&max=<?= $max ?>&page=<?= $i?>'><?= $i?></a>
                    </li>
                    <?php }

                    if($page>=1 && $page != $tongsotrang){
                        echo"<li><a href='index.php?page_layout=LocGia&min=$min&max=$max&page=".($page+1)."'>&raquo;</a></li>";
                    }
                ...
                <?php
                    ...
                    echo '<p class="lead"><em>Không tìm thấy kết quả cho "'.number_format($min)." -> ".number_format($max).'".</em></p>';
                }
